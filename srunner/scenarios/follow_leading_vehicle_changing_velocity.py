...
class FollowLeadingVehicleChangingVelocityAndStop(BasicScenario):
    ...
    def _create_behavior(self):
        ...
        def _drive_for_distance(composite, target_speed, distance):
            drive = py_trees.composites.Parallel(
                "DrivingForDistance",
                policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
            drive.add_child(WaypointFollower(self.other_actors[0], target_speed))
        ...
        start_transform = ActorTransformSetter(other_actor, self._other_actor_transform)
        ...
        straight_at_first_intersection = WaypointFollower(other_actor, self._first_vehicle_speed, plan=plan)
        ...
        drive_2 = py_trees.composites.Sequence("Drive straight while changing velocity")
        drive_2.add_child(drive_1)
        drive_2.add_child(BrakeVehicle(other_actor, 0.2 , 1))
        _drive_for_distance(drive_2, 8, 30)
        drive_2.add_child(BrakeVehicle(other_actor, 0.2 , 1))
        _drive_for_distance(drive_2, 8, 20)
